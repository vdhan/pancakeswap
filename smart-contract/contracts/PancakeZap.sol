 ...
 */
contract PancakeZap is Ownable, ReentrancyGuard {
    ...
    uint public constant MINIMUM_AMOUNT = 1000;

    // Maximum reverse zap ratio (100 --> 1%, 1000 --> 0.1%)
    uint public maxZapReverseRatio;
    ...
    event AdminTokenRecovery(address indexed tokenAddress, uint amountTokens);

    // Owner changes the maxZapReverseRatio
    event NewMaxZapReverseRatio(uint maxZapReverseRatio);
     ...
     */
    constructor(address _WBNBAddress, address _pancakeRouter, uint _maxZapReverseRatio) {
     ...
     * @param _tokenAmountOutMin: minimum token amount (e.g. CAKE) to receive in the intermediary swap
     *  (e.g. BNB --> CAKE)
     */
    function zapInBNB(address _lpToken, uint _tokenAmountOutMin) external payable nonReentrant {
        WBNB.deposit{value: msg.value}();

        // Call zap function
        uint lpTokenAmountTransferred = _zapIn(WBNBAddress, msg.value, _lpToken, _tokenAmountOutMin);
     ...
     */
    function zapInToken(address _tokenToZap, uint _tokenAmountIn, address _lpToken, uint _tokenAmountOutMin)
        external
        nonReentrant
    {
        ...
        // Call zap function
        uint lpTokenAmountTransferred = _zapIn(_tokenToZap, _tokenAmountIn, _lpToken, _tokenAmountOutMin);
    ...
    function zapInTokenRebalancing(
        ...
        uint _token0AmountIn,
        uint _token1AmountIn,
        address _lpToken,
        uint _tokenAmountInMax,
        uint _tokenAmountOutMin,
        bool _isToken0Sold
    )
        external
        nonReentrant
    {
        ...
        // Call zapIn function
        uint lpTokenAmountTransferred = _zapInRebalancing(
            _token0ToZap,
            _token1ToZap,
            _token0AmountIn,
            _token1AmountIn,
     ...
     */
    function zapOutBNB(address _lpToken, uint _lpTokenAmount, uint _tokenAmountOutMin)
        external
        nonReentrant
    {
        ...
        // Call zapOut
        uint tokenAmountToTransfer = _zapOut(_lpToken, WBNBAddress, _tokenAmountOutMin);
     ...
     * @param _tokenAmountOutMin: minimum token to receive (e.g. CAKE) in the intermediary swap (e.g. BUSD --> CAKE)
     */
    function zapOutToken(address _lpToken, address _tokenToReceive, uint _lpTokenAmount, uint _tokenAmountOutMin)
        external
        nonReentrant
    {
        // Transfer LP token to this address
        IERC20(_lpToken).safeTransferFrom(address(msg.sender), address(_lpToken), _lpTokenAmount);
        uint tokenAmountToTransfer = _zapOut(_lpToken, _tokenToReceive, _tokenAmountOutMin);
     ...
     */
    function updateMaxZapInverseRatio(uint _maxZapInverseRatio) external onlyOwner {
     ...
     */
    function estimateZapInSwap(address _tokenToZap, uint _tokenAmountIn, address _lpToken)
        external
        view
        returns (uint swapAmountIn, uint swapAmountOut, address swapTokenOut)
    {
        ...
        // Convert to uint (from uint112)
        (uint reserveA, uint reserveB,) = IPancakePair(_lpToken).getReserves();
    ...
    function estimateZapInRebalancingSwap(
        ...
        external
        view
        returns (uint swapAmountIn, uint swapAmountOut, bool sellToken0)
    {
        require(
     ...
     */
    function estimateZapOutSwap(address _lpToken, uint _lpTokenAmount, address _tokenToReceive)
        external
        view
        returns (uint swapAmountIn, uint swapAmountOut, address swapTokenOut)
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint tokenAmountIn = (_lpTokenAmount * reserveA) / IPancakePair(_lpToken).totalSupply();
            swapAmountIn = _calculateAmountToSwap(tokenAmountIn, reserveA, reserveB);
            swapAmountOut = pancakeRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
            swapTokenOut = token0;
        } else {
            // sell token1
            uint tokenAmountIn = (_lpTokenAmount * reserveB) / IPancakePair(_lpToken).totalSupply();
     ...
     * @param _tokenAmountOutMin: minimum token to receive in the intermediary swap
     */
    function _zapIn(address _tokenToZap, uint _tokenAmountIn, address _lpToken, uint _tokenAmountOutMin)
        internal
        returns (uint lpTokenReceived)
    {
        ...
        uint[] memory swapedAmounts = pancakeRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
    ...
    function _zapInRebalancing(
        ...
        uint _tokenAmountOutMin,
        bool _isToken0Sold
    ) internal returns (uint lpTokenReceived) {
        require(
            _token0ToZap == IPancakePair(_lpToken).token0() || _token0ToZap == IPancakePair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
     ...
     * @param _tokenAmountOutMin: minimum token to receive in the intermediary swap
     */
    function _zapOut(address _lpToken, address _tokenToReceive, uint _tokenAmountOutMin) internal returns (uint) {
        address token0 = IPancakePair(_lpToken).token0();
        address token1 = IPancakePair(_lpToken).token1();
        require(_tokenToReceive == token0 || _tokenToReceive == token1, "Zap: Token not in LP");

        // Burn all LP tokens to receive the two tokens to this address
        (uint amount0, uint amount1) = IPancakePair(_lpToken).burn(address(this));
     ...
     */
    function _calculateAmountToSwap(uint _token0AmountIn, uint _reserve0, uint _reserve1)
        private
        view
        returns (uint amountToSwap)
    {
        uint halfToken0Amount = _token0AmountIn / 2;
        uint nominator = pancakeRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint denominator = pancakeRouter.quote(halfToken0Amount, _reserve0 + halfToken0Amount, _reserve1 - nominator);

        // Adjustment for price impact
        amountToSwap = _token0AmountIn - Babylonian.sqrt(
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        uint _reserve1,
        bool _isToken0Sold
    ) private view returns (uint amountToSwap) {
        ...
        if (sellToken0) {
            uint token0AmountToSell = (_token0AmountIn - (_token1AmountIn * _reserve0) / _reserve1) / 2;
            uint nominator = pancakeRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint denominator = pancakeRouter.quote(
                token0AmountToSell,
            ...
            uint token1AmountToSell = (_token1AmountIn - (_token0AmountIn * _reserve1) / _reserve0) / 2;
            uint nominator = pancakeRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
            uint denominator = pancakeRouter.quote(
                token1AmountToSell,
            ...
            // Adjustment for price impact
            amountToSwap = 2 * token1AmountToSell - Babylonian.sqrt(
